...
**Intervalos de Confianza de los modelos**

Dado que el modelo logístico modela el logaritmo de ODDs, estas son las unidades en las que se devuelven predicciones.Es necesario convertilas de nuevo a probabilidad mediante la función logit. En R la función predict() puede devolver directamente las probabilidades en lugar de los logODDs si se indica el argumento type="response". Pero para calcular intervalos de confianza y que éstos no se salgan del rango [0,1], es necesario emplear los logODDs y una vez sustraído o sumado el margen de error, se transforman a probabilidades.
...
#Modelo modelo.log.5
intervalo_mod_5<-confint(object = modelo.log.5,level = 0.95)
intervalo_mod_5


#Modelo modelo.log.7
intervalo_mod_7<-confint(object = modelo.log.7,level = 0.95)
intervalo_mod_7
```
